...
<template>
    <div v-if="showProfile" class="flex h-full items-center">
        <Menu v-if="currentDesign" v-slot="{open}" as="div" class="relative inline-block text-left w-full">
            <div class="flex items-center space-x-1">
                <MenuButton>
                    <div class="w-full flex justify-between items-center border rounded py-1 px-2">
                        ...
                        <div v-else-if="!isCustomApi" class="flex items-start flex-col text-xs ml-2">
                            <div class="leading-0 w-20 text-left">Unknown</div>
                            <div @click.stop="handleSignIn" class="leading-0 text-2xs underline">{{ $t('Sign In') }}</div>
                        </div>
            ...
            <transition v-if="!loadingDesign && permissions.canReorder" enter-active-class="transition ease-out duration-100"
                ...
                <MenuItems class="absolute right-0 z-50 mt-1 w-full xl:w-48 origin-top-right bg-builder rounded-md shadow-lg ring-1 ring-black ring-opacity-5 focus:outline-none">
                    <div class="p-2">
                            ...
                            <MenuItem class="hover:bg-opacity-20 px-4 py-2">
                                <div v-if="customer">
                                    <div v-if="customer.email" @click="handleSwitchAccount" class="text-left text-sm flex hover:underline cursor-pointer items-center">
                                        <svg-icon type="mdi" :path="mdiSwapHorizontal" size="20" class="mr-2"/>
                                        Switch Account
                                    </div>
                                </div>
                                <div v-else @click="handleSignIn" class="text-left flex items-center text-sm cursor-pointer">
                                    <svg-icon type="mdi" :path="mdiAccountCircleOutline" size="20" class="mr-2"/>
                                    {{ $t('Sign In') }}
                                </div>
                            </MenuItem>
                            <hr/>
                        ...
                        <MenuItem v-if="customer && isStandalone" class="hover:bg-opacity-20  px-4 py-2">
                            <div class="text-left flex items-center text-sm cursor-pointer hover:underline" @click="handleLogOut">
                                <svg-icon type="mdi" :path="mdiLogout" size="16" class="mr-2"/>
                                {{ $t('Log Out') }}
                            </div>
                        </MenuItem>
                    </div>
