...
<template>
    <Menu as="div" class="relative inline-block text-left w-full h-full">
        <div class="h-full w-full" :class="{'pointer-events-none': disabled}">
            <menu-button
                class="w-full min-w-[100px] justify-between gap-x-1.5 h-full flex items-center rounded-md px-3 py-1 box-border text-sm border border-gray-300">
        ...
        <transition v-if="!disabled" enter-active-class="transition ease-out duration-100"
            ...
            <menu-items class="absolute left-0 z-50 mt-1 w-56 origin-top-right rounded-lg bg-builder focus:outline-none border border-gray-300">
